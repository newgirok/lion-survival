...
export function pause(scene, type) {
  ...
  // (2) pause가 해제된 후 최소 100ms가 지나야 다시 pause 할 수 있도록 하는 역할을 합니다.
...
document.addEventListener("keydown", (event) => {
  if (
    ...
    // `scene`을 사용하지 않고 `game.scene.getScene(scene)`을 사용한 이유
    // scene이 pause되면 해당 scene의 변수, 함수 등을 사용할 수 없게 되기 때문입니다.
